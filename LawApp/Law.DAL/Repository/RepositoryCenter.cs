...
{
    public class RepositoryCenter : RepositoryBase<Center>, IRepositorySpecial<Center>
    {
        public RepositoryCenter(LawAppDbContext context) : base(context)
        {
        }

        public async Task<Center> GetMax()
        {
            var maxStage = await _context.centers.OrderByDescending(n => n.Code).FirstOrDefaultAsync();
            return maxStage;
        }

        public int GetMaxIdOfItem()
        {
            var id = _context.centers.Max(i => i.Id);
            return id;
        }

        public async Task<Center> GetMin()
        {
            var minSatge = await _context.centers.OrderBy(n => n.Code).FirstOrDefaultAsync();
            return minSatge;
        }
        ...
        public async Task<int> GetNewCode()
        {
            try
            {
                ...
                int? maxCode = await _context.centers
                    .Select(c => (int?)c.Code)
        ...
        public async Task<Center> GetNextOrPreviousItemByCode(int id, string direction)
        {
            // Get the current record's code.
            var currentCode = await _context.centers
                .Where(n => n.Id == id)
                .Select(n => n.Code)
                .FirstOrDefaultAsync();
            ...
            if (!string.IsNullOrEmpty(direction))
            {
                if (direction.ToLower() == "previous")
                {
                    ...
                    var previousStage = await _context.centers
                ...
                else if (direction.ToLower() == "next")
                {
                    ...
                    var nextStage = await _context.centers
                        .Where(n => n.Code > currentCode)
                        .OrderBy(n => n.Code)
                        .FirstOrDefaultAsync();
                    return nextStage;
