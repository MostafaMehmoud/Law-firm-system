...
{
    public class RepositoryClient : RepositoryBase<Client>, IRepositorySpecial<Client>
    {
        public RepositoryClient(LawAppDbContext context) : base(context)
        {
        }

        public async Task<Client> GetMax()
        {
            var maxStage = await _context.clients.OrderByDescending(n => n.Code).FirstOrDefaultAsync();
            return maxStage;
        }

        public int GetMaxIdOfItem()
        {
            var id = _context.clients.Max(i => i.Id);
            return id;
        }

        public async Task<Client> GetMin()
        {
            var minSatge = await _context.clients.OrderBy(n => n.Code).FirstOrDefaultAsync();
            return minSatge;
        }
        ...
        public async Task<int> GetNewCode()
        {
            try
            {
                ...
                int? maxCode = await _context.clients
                    .Select(c => (int?)c.Code)
        ...
        public async Task<Client> GetNextOrPreviousItemByCode(int id, string direction)
        {
            // Get the current record's code.
            var currentCode = await _context.clients
                .Where(n => n.Id == id)
                .Select(n => n.Code)
                .FirstOrDefaultAsync();
            ...
            if (!string.IsNullOrEmpty(direction))
            {
                if (direction.ToLower() == "previous")
                {
                    ...
                    var previousStage = await _context.clients
                ...
                else if (direction.ToLower() == "next")
                {
                    ...
                    var nextStage = await _context.clients
                        .Where(n => n.Code > currentCode)
                        .OrderBy(n => n.Code)
                        .FirstOrDefaultAsync();
                    return nextStage;
