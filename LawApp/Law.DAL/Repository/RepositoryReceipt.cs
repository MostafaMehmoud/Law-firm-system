...
{
    internal class RepositoryReceipt : RepositoryBase<Receipt>, IRepositorySpecial<Receipt>
    {
        public RepositoryReceipt(LawAppDbContext context) : base(context)
        {
        }

        public async  Task<Receipt> GetMax()
        {
            var maxStage = await _context.receipts.OrderByDescending(n => n.Code).FirstOrDefaultAsync();
            return maxStage;
        }
        ...
        {
            var minSatge = await _context.receipts.OrderBy(n => n.Code).FirstOrDefaultAsync();
            return minSatge;
        }
        ...
        public async Task<int> GetNewCode()
        {
            try
            {
                ...
                int? maxCode = await _context.receipts
                    .Select(c => (int?)c.Code)
        ...
        public async Task<Receipt> GetNextOrPreviousItemByCode(int id, string direction)
        {
            ...
            if (!string.IsNullOrEmpty(direction))
            {
                if (direction.ToLower() == "previous")
                {
                    ...
                    var previousStage = await _context.receipts
                ...
                else if (direction.ToLower() == "next")
                {
                    ...
                    var nextStage = await _context.receipts
                        .Where(n => n.Code > currentCode)
                        .OrderBy(n => n.Code)
                        .FirstOrDefaultAsync();
                    return nextStage;
