...
{
    public class RepositoryIssues : RepositoryBase<Issue>, IRepositorySpecial<Issue>
    {
        public RepositoryIssues(LawAppDbContext context) : base(context)
        {
        }

        public async Task<Issue> GetMax()
        {
            var maxStage = await _context.issues.OrderByDescending(n => n.Code).FirstOrDefaultAsync();
            return maxStage;
        }

        public int GetMaxIdOfItem()
        {
            var id = _context.issues.Max(i => i.Id);
            return id;
        }

        public async Task<Issue> GetMin()
        {
            var minSatge = await _context.issues.OrderBy(n => n.Code).FirstOrDefaultAsync();
            return minSatge;
        }
        ...
        public async Task<int> GetNewCode()
        {
            try
            {
                ...
                int? maxCode = await _context.issues
                    .Select(c => (int?)c.Code)
        ...
        public async Task<Issue> GetNextOrPreviousItemByCode(int id, string direction)
        {
            // Get the current record's code.
            var currentCode = await _context.issues
                .Where(n => n.Id == id)
                .Select(n => n.Code)
                .FirstOrDefaultAsync();
            ...
            if (!string.IsNullOrEmpty(direction))
            {
                if (direction.ToLower() == "previous")
                {
                    ...
                    var previousStage = await _context.issues
                ...
                else if (direction.ToLower() == "next")
                {
                    ...
                    var nextStage = await _context.issues
                        .Where(n => n.Code > currentCode)
                        .OrderBy(n => n.Code)
                        .FirstOrDefaultAsync();
                    return nextStage;
