...
{
    public class RepositoryIssueFile : RepositoryBase<IssueFile>, IRepositorySpecial<IssueFile>
    {
        public RepositoryIssueFile(LawAppDbContext context) : base(context)
        {
        }

        public async Task<IssueFile> GetMax()
        {
            var maxStage = await _context.issuesFiles.OrderByDescending(n => n.Code).FirstOrDefaultAsync();
            return maxStage;
        }

        public int GetMaxIdOfItem()
        {
            var id = _context.issuesFiles.Max(i => i.Id);
            return id;
        }

        public async Task<IssueFile> GetMin()
        {
            var minSatge = await _context.issuesFiles.OrderBy(n => n.Code).FirstOrDefaultAsync();
            return minSatge;
        }
        ...
        public async Task<int> GetNewCode()
        {
            try
            {
                ...
                int? maxCode = await _context.issuesFiles
                    .Select(c => (int?)c.Code)
        ...
        public async Task<IssueFile> GetNextOrPreviousItemByCode(int id, string direction)
        {
            // Get the current record's code.
            var currentCode = await _context.issuesFiles
                .Where(n => n.Id == id)
                .Select(n => n.Code)
                .FirstOrDefaultAsync();
            ...
            if (!string.IsNullOrEmpty(direction))
            {
                if (direction.ToLower() == "previous")
                {
                    ...
                    var previousStage = await _context.issuesFiles
                ...
                else if (direction.ToLower() == "next")
                {
                    ...
                    var nextStage = await _context.issuesFiles
                        .Where(n => n.Code > currentCode)
                        .OrderBy(n => n.Code)
                        .FirstOrDefaultAsync();
                    return nextStage;
