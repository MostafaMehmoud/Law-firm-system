...
{
    public class RepositoryCourt : RepositoryBase<Court>, IRepositorySpecial<Court>
    {
        public RepositoryCourt(LawAppDbContext context) : base(context)
        {
        }

        public async Task<Court> GetMax()
        {
            var maxStage = await _context.courts.OrderByDescending(n => n.Code).FirstOrDefaultAsync();
            return maxStage;
        }

        public int GetMaxIdOfItem()
        {
            var id = _context.courts.Max(i => i.Id);
            return id;
        }

        public async Task<Court> GetMin()
        {
            var minSatge = await _context.courts.OrderBy(n => n.Code).FirstOrDefaultAsync();
            return minSatge;
        }
        ...
        public async Task<int> GetNewCode()
        {
            try
            {
                ...
                int? maxCode = await _context.courts
                    .Select(c => (int?)c.Code)
        ...
        public async Task<Court> GetNextOrPreviousItemByCode(int id, string direction)
        {
            // Get the current record's code.
            var currentCode = await _context.courts
                .Where(n => n.Id == id)
                .Select(n => n.Code)
                .FirstOrDefaultAsync();
            ...
            if (!string.IsNullOrEmpty(direction))
            {
                if (direction.ToLower() == "previous")
                {
                    ...
                    var previousStage = await _context.courts
                ...
                else if (direction.ToLower() == "next")
                {
                    ...
                    var nextStage = await _context.courts
                        .Where(n => n.Code > currentCode)
                        .OrderBy(n => n.Code)
                        .FirstOrDefaultAsync();
                    return nextStage;
