...
{
    public class RepositoryParty : RepositoryBase<Party>, IRepositorySpecial<Party>
    {
        public RepositoryParty(LawAppDbContext context) : base(context)
        {
        }

        public async Task<Party> GetMax()
        {
            var maxStage = await _context.parties.OrderByDescending(n => n.Code).FirstOrDefaultAsync();
            return maxStage;
        }

        public int GetMaxIdOfItem()
        {
            var id = _context.parties.Max(i => i.Id);
            return id;
        }

        public async Task<Party> GetMin()
        {
            var minSatge = await _context.parties.OrderBy(n => n.Code).FirstOrDefaultAsync();
            return minSatge;
        }
        ...
        public async Task<int> GetNewCode()
        {
            try
            {
                ...
                int? maxCode = await _context.parties
                    .Select(c => (int?)c.Code)
        ...
        public async Task<Party> GetNextOrPreviousItemByCode(int id, string direction)
        {
            // Get the current record's code.
            var currentCode = await _context.parties
                .Where(n => n.Id == id)
                .Select(n => n.Code)
                .FirstOrDefaultAsync();
            ...
            if (!string.IsNullOrEmpty(direction))
            {
                if (direction.ToLower() == "previous")
                {
                    ...
                    var previousStage = await _context.parties
                ...
                else if (direction.ToLower() == "next")
                {
                    ...
                    var nextStage = await _context.parties
                        .Where(n => n.Code > currentCode)
                        .OrderBy(n => n.Code)
                        .FirstOrDefaultAsync();
                    return nextStage;
